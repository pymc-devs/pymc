...
class TestScalarParameterSamples(SeededTest):
    ...
    def test_truncated_normal_lower(self):
        def ref_rand(size, mu, sigma, lower):
            return st.truncnorm.rvs(
                (lower - mu) / sigma, np.inf, size=size, loc=mu, scale=sigma
            )
    ...
    def test_truncated_normal_upper(self):
        def ref_rand(size, mu, sigma, upper):
            return st.truncnorm.rvs(
                -np.inf, (upper - mu) / sigma, size=size, loc=mu, scale=sigma
            )
    ...
    def test_skew_normal(self):
        ...
        pymc3_random(
            pm.SkewNormal, {"mu": R, "sigma": Rplus, "alpha": R}, ref_rand=ref_rand
        )
    ...
    def test_triangular(self):
        ...
        pymc3_random(
            pm.Triangular,
            {"lower": Runif, "upper": Runif + 3, "c": Runif + 1},
            ref_rand=ref_rand,
        )
...
class TestDensityDist:
    ...
    def test_density_dist_with_random_sampleable(self, shape):
        ...
        ppc = pm.sample_posterior_predictive(
            trace, samples=samples, model=model, size=size
        )
    ...
    def test_density_dist_with_random_sampleable_failure(self, shape):
        ...
        with pytest.raises(RuntimeError):
            pm.sample_posterior_predictive(
                trace, samples=samples, model=model, size=100
            )
    ...
    def test_density_dist_without_random_not_sampleable(self):
        with pm.Model() as model:
            ...
            normal_dist = pm.Normal.dist(mu, 1)
            pm.DensityDist(
                "density_dist", normal_dist.logp, observed=np.random.randn(100)
            )
            trace = pm.sample(100)
