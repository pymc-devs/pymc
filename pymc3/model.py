...
class Model(Context, Factor, WithMemoization, metaclass=InitContextMeta):
    ...
    def predict(self, trace, dict_new_values, **kwargs):
        """Predicts output data given a new set of input data.
        ...
        dict_new_values : dict
            ...
            Before returning from the function, the variables return to their
            original values.
        **kwargs : keyword arguments
        ...
        -------
        ppc : dict
            Output of the function pymc3.sample_posterior_predictive() using
            new values for the data containers.
            Dictionary with the variables as keys. The values corresponding to
            the posterior predictive samples from the weighted models.
        ...
        .. code:: ipython
            ...
            >>> # Predict on new data.
            >>> y_test = model.predict(trace, {'x': [5,6,9]})
            >>> y_test['obs'].mean(axis=0)
            array([4.6088569 , 5.54128318, 8.32953844])
        """
        dict_original_values = {}
        for variable_name, new_value in dict_new_values.items():
            original_value = self[variable_name].get_value()
            dict_original_values[variable_name] = original_value
            self[variable_name].set_value(pandas_to_array(new_value))

        ppc = pm.sample_posterior_predictive(trace=trace, model=self, **kwargs)

        for variable_name, original_value in dict_original_values.items():
            self[variable_name].set_value(original_value)

        return ppc

    def refit(self, dict_new_values, **kwargs):
        ...
        **kwargs : keyword arguments
        ...
        .. code:: ipython
            ...
            ...     new_trace = model.refit({'data' : data_vals})
            ...     traces.append(new_trace)
        """
        for variable_name, new_value in dict_new_values.items():
            self[variable_name].set_value(pandas_to_array(new_value))

        trace = pm.sample(model=self, **kwargs)
