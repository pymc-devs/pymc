...
class Marginal(Base):
    ...
    def marginal_likelihood(self, name, X, y, noise, is_observed=True, n_outputs=None, **kwargs):
        R"""
    ...
    def _build_conditional(self, Xnew, pred_noise, diag, X, y, noise, cov_total, mean_total):
        ...
        mu = self.mean_func(Xnew) + aet.dot(A.T, v).T
        if diag:
    ...
    def conditional(self, name, Xnew, pred_noise=False, given=None, n_outputs=None, **kwargs):
        R"""
        Returns the conditional distribution evaluated over new input
        locations `Xnew`.
...
class MarginalSparse(Marginal):
    ...
    def _build_conditional(self, Xnew, pred_noise, diag, X, Xu, y, sigma, cov_total, mean_total):
        ...
        if diag:
            ...
            cov = self.cov_func(Xnew) - aet.dot(aet.transpose(As), As) + aet.dot(aet.transpose(C), C)
            if pred_noise:
