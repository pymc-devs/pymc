...
class Metropolis(ArrayStepShared):
    ...
    def __init__(self, vars=None, S=None, proposal_dist=None, scaling=1.,
        ...
        ## CHANGED: SS because S is taken
        #vars = pm.inputvars(vars)
        vars = SS.inputvars(vars)
        ...
        shared = SS.make_shared_replacements(vars, model)

        ## CHANGED: Ended up making the delta log p function in the backendselfself.
        ## I'm just using two calls to logp for the tensorflow implementation.
        self.delta_logp = SS.delta_logp(model.logpt, vars, shared)
        self.logp = SS.logp(model.logpt,vars,shared)
    ...
    def astep(self, q0):
        ...
        if SS.backend()=='theano':
            ...
            if self.any_discrete:
                ...
                ## CHANGED.
                q = SS.floatX(q0+delta)

            accept = self.delta_logp(q, q0)

        ## CHANGED: Did a slightly different implementation in tensorflow.
        ## There is probably better way of doing this
        elif SS.backend()=='tensorflow':
            ...
            # the other variables have the same value.

            # calculate logp for each point rather than the difference
            # (this was easier to do first in TensorFlow)
            accept = np.squeeze(np.array(self.logp(q))-np.array(self.logp(q0)))
