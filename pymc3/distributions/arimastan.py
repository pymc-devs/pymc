...
class Sarima:
    def __init__(self, n, n1, p, d, q, pp, dd, qq, period, d1, sigma0, mu0):
        # Model data
    ...
    def prior_for_mu0(self, target, prior_for_mu0):
        ...
        if self.prior_mu0[4] == 4:
            target += pm.StudentT(
                nu=self.prior_mu0[3], mu=self.prior_mu0[1], sigma=self.prior_mu0[2]
            ).logp(self.mu0)
        if self.prior_mu0[4] == 5:
            target += pm.Cauchy(alpha=self.prior_mu0[1], beta=self.prior_mu0[2]).logp(self.mu0)
        if self.prior_mu0[4] == 6:
            target += pm.InverseGamma(alpha=self.prior_mu0[1], beta=self.prior_mu0[2]).logp(
                self.mu0
            )
        if self.prior_mu0[4] == 7:
            target += invchi(self.mu0, pm.ChiSquared(nu=self.prior_mu0[3]))  # inverse chi square
        ...
        if self.prior_sigma0[4] == 1:
            target += pm.Normal(mu=self.prior_sigma0[1], sigma=self.prior_sigma0[2]).logp(
                self.sigma0
            )
        if self.prior_sigma0[4] == 2:
            target += pm.Beta(alpha=self.prior_sigma0[1], beta=self.prior_sigma0[2]).logp(
                self.sigma0
            )
        if self.prior_sigma0[4] == 3:
            target += pm.Uniform(lower=self.prior_sigma0[1], upper=self.prior_sigma0[2]).logp(
                self.sigma0
            )
        if self.prior_sigma0[4] == 4:
            target += pm.StudentT(
                nu=self.prior_sigma0[3], mu=self.prior_sigma0[1], sigma=self.prior_sigma0[2]
            ).logp(self.sigma0)
        if self.prior_sigma0[4] == 5:
            target += pm.Cauchy(alpha=self.prior_sigma0[1], beta=self.prior_sigma0[2]).logp(
                self.sigma0
            )
        if self.prior_sigma0[4] == 6:
            target += pm.InverseGamma(alpha=self.prior_sigma0[1], beta=self.prior_sigma0[2]).logp(
                self.sigma0
            )
        if self.prior_sigma0[4] == 7:
            target += invchi(self.sigma0, pm.ChiSquared(nu=self.prior_sigma0[3]))
        if self.prior_sigma0[4] == 8:
            target += -np.log(self.sigma0)
        if self.prior_sigma0[4] == 9:
            target += pm.Gamma(alpha=self.prior_sigma0[1], beta=self.prior_sigma0[2]).logp(
                self.sigma0
            )
