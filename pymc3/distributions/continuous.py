...
def get_tau_sd(tau=None, sd=None):
    ...
    ## CHANGED:
    return (S.floatX(tau), S.floatX(sd))
...
#         self.sd = sd = as_tensor_variable(sd)
#         self.tau = tau = as_tensor_variable(tau)
#
#         #self.mean = tt.sqrt(2 / (np.pi * self.tau))
#         self.mean = sqrt(2 / (np.pi * self.tau))
...
#         #              value >= 0,
#         #              tau > 0, sd > 0)
#         return bound(-0.5 * tau * value**2 + 0.5 * log(tau * 2. / np.pi),
#                      value >= 0,
#                      tau > 0, sd > 0)
#
#     def _repr_latex_(self, name=None, dist=None):
