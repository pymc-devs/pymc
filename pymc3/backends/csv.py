"""CSV file trace backend

Store and retrieve sampling values from CSV file.
...
class CSV(base.BaseTrace):
    ...
    def __init__(self, name, model=None, vars=None):
        if not os.path.exists(name):
            os.mkdir(name)
        super(CSV, self).__init__(name, model, vars)
    ...
    def _slice(self, idx):
        warnings.warn('Slice for CSV backend has no effect.')
...
def load(name, model=None):
    ...
    ----------
    name : str
        Name of directory that stores CSV files.
    model : Model
        If None, the model is taken from the `with` context.
