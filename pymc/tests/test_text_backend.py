...
class TestTextWrite(TextTestCase):
    ...
    def test_close_args(self):
        ...
        for call, varname in enumerate(trace.varnames):
            fname, data = self.savetxt.call_args_list[call][0]
            self.assertEqual(fname, 'textdb/chain-0/{}.txt'.format(varname))
            npt.assert_equal(data, trace[varname].reshape(-1, data.size))
    ...
    def test_close_shape(self):
        ...
        expected = {varname: [self.draws] + list(var_shape)
                    for varname, var_shape in trace.var_shapes.items()}
...
class TestTextLoad(TextTestCase):
    ...
    def test_load_model_supplied_scalar(self):
        ...
        mtrace = text.load('textdb', model=self.model)
        npt.assert_equal(mtrace.get_values('x', chains=[0]), data)
    ...
    def test_load_model_supplied_1d(self):
        ...
        mtrace = text.load('textdb', model=self.model)
        npt.assert_equal(mtrace['x'], data)
    ...
    def test_load_model_supplied_multichain_chains(self):
        ...
        self.assertEqual(mtrace.chains, [0, 1])
    ...
    def test_load_model_supplied_multichain_chains_select_one(self):
        ...
        mtrace = text.load('textdb', model=self.model, chains=[1])

        self.assertEqual(mtrace.chains, [1])
