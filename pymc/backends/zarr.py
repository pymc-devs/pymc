    ...
    def record(self, draw: Mapping[str, np.ndarray], stats: Sequence[Mapping[str, Any]]):
        ...
        for var_name, var_value in zip(self.varnames, self.fn(draw)):
            if var_name in unconstrained_variables:
                self._unconstrained_posterior[var_name].set_orthogonal_selection(
                    (chain, draw_idx),
                    var_value,
                )
            else:
...
class ZarrTrace:
    ...
    def split_warmup(self, group_name, error_if_already_split=True):
    ...
    def to_inferencedata(self, save_warmup=False) -> az.InferenceData:
