...
Like any other random variable, a GP can, at least conceptually, be incorporated in a Bayesian probability model and its value can be inferred from data. Equation~\ref{regprior} provides a simple example: unknown function $f$ is assigned a GP prior, and the dependence of data $d$ on it is modeled as follows:
\begin{eqnarray*}
    \textup{data}_i \stackrel{\tiny{\textup{ind}}}{\sim} \textup{N}(f(o_i), V_i)
    f \sim \textup{GP}(M,C).
\end{eqnarray*}
...
For more general models, the posterior of $f$ is usually not of a standard form. As a concrete example, this section and the next will consider the nonparametric regression model implemented in \code{pymc/examples/gp/PyMCmodel.py}, which is a simple hierarchical extension of the model above. In the hierarchical model, the mean $M_{a,b,c}$ is a parabola. The covariance $C_{\nu,\theta,\phi}$ of $f$ is the Mat\`ern covariance with degree of differentiability parameter $\nu$, with distance scaled by $\theta$, and multiplied by $\phi^2$ so that $\mathsf{V}(f(x))=\phi^2$. The data $d$ are noisy measurements of $f$ at observation locations $o$. The full model specification, including priors for the scalar parameters, is
\begin{eqnarray*}
    ...
    d_i|f \stackrel{\textup{\tiny ind}}{\sim} \textup{Normal}(f(o_i), .0001)
\end{eqnarray*}
For this model, the posterior of $f$ does not have a standard form. In Bayesian analysis, nonstandard posteriors are commonly sampled using Markov chain Monte Carlo methods, but development of such methods is complicated by the fact that unknown functions on infinite domains (such as $\mathbb R$) are infinite-dimensional random variables.
...
In \code{pymc/examples/gp/PyMCmodel.py}, a GP submodel that contains $f$ in the model above is created as follows:
\begin{CodeChunk}
\begin{CodeInput}
...
Once the GP submodel has been created, other variables can depend on $f$ and $f(x_*)$ in any way that can be expressed in \proglang{Python} code. In the model above, the observation $d$ depends on $f(o)$, where $o$ are the observation locations. If $x_*$ is chosen to be $o$, then $d$ depends on $f(x_*)$, and no other evaluations of $f$. As discussed in section \ref{sec:step-methods}, this parameterization is usually computationally advantageous and should be used if the observation locations $o$ are known a priori. It is used in \code{PyMCmodel.py}: 
\begin{CodeChunk}
\begin{CodeInput}
d = pymc.Normal('d',mu=sm.f_eval, tau=1./V, value=init_val, observed=True)
\end{CodeInput}
\end{CodeChunk}
In Section~\ref{sec:duffy}, a \code{GaussianProcess}'s children depend on it in a less standard way, although the observation locations are still known a priori. 
...
This section describes this package's Metropolis-Hastings rule for updating the parents of GPs jointly with the GPs themselves. In the context of the model above, the GP itself is $f$ and the parents are $a$, $b$, $c$, $\nu$, $\phi$ and $\theta$. 

The problem that this updating rule overcomes is that it is impractical to associate a probability density function with GPs, which would be needed to compute Metropolis-Hastings acceptance ratios. The updating rule is developed heuristically by assuming the false premise that the probability density functions are available, then choosing the proposal distribution so as to avoid the need to evaluate them. The development can be made more rigorous by replacing $f$ with its evaluation at all the points at which its value would ever be needed.
...
\end{eqnarray*}
Since no density is available for the unknown functions $f$ and $f_p$, the terms with these variables in the consequent position cannot be evaluated:
\begin{eqnarray*}
...
\end{eqnarray*}
In other words, if $f$ is proposed from its prior distribution conditional on $f(x_*)$ and its parents whenever $f(x_*)$ is proposed, the intractable terms don't have to be computed.
...
The Duffy negativity phenotype, as modelled by \cite{Howes}, is controlled by two genetic loci. The first contains one of two alleles, known as \emph{FY*A} and \emph{FY*B}. These alleles encode proteins known as Fy$^\textup{a}$ and Fy$^\textup{b}$, respectively. The second locus can contain an allele that prevents expression of these proteins. A person is called `Duffy negative' if they do not express either protein Fy$^\textup{a}$ or Fy$^\textup{b}$, that is if each of their two copies of chromosome 1 contains the silencing allele. 

For clarity, in the remainder of this section the event that \emph{FY*A} is present at the first locus in a given chromosome will be denoted $a$, and the event that \emph{FY*B} is present by $b$. Events $a$ and $b$ are, for the purposes of this model, mutually exclusive and all-inclusive. Similarly, the event that the silencing allele is present at the second locus will be denoted $s$. The genotype of a given copy of chromosome 1 will be denoted by juxtaposing the events at the two loci: $as$, $b\neg s$, etc. The genotype of a person will be denoted as an unordered pair of chromosomal genotypes: $(bs,a \neg s)$, etc.
...
\begin{table}
    ...
    \begin{small}
        ...
        \end{tabular}            
    \end{small}
    \caption{The ability of the five tests used by data collectors in the Duffy negativity example to distinguish between the ten possible genotypes. A survey type is able to distinguish between genotypes if the numbers in their columns are different.}
    \label{tab:surveys}  
\end{table}
...
The model contains two GPs, denoted $f_b$ and $f_s$. Because the observation locations $o$ are known a priori, they are used as the mesh for both. `Noisy' versions $\tilde f_b$ and $\tilde f_s$ are generated by adding an iid normal random variate at each location $x$ on the surface of the earth. The variances of these noisy versions are labelled $V_b$ and $V_s$, respectively.

A nonlinear inverse logit transformation of $\tilde f_b$ gives $p(b;x)$, the probability that event $b$ occurs in a randomly-chosen copy of chromosome 1 from location $x$. The same transformation of $\tilde f_s$ gives $p(s;x|b)$, the probability that event $s$ occurs in a randomly-chosen copy of chromosome 1 from location $x$ given than event $b$ occurs in the same chromosome. The corresponding probability $p(s;x|a)$, the probability that event $s$ occurs in a randomly-chosen copy of chromosome 1 from location $x$ given that event $a$ occurs in the same chromosome, is modelled as a simple unknown constant $p_1$ rather than a transformed GP because the event $as$ is quite rare; $s$ is found almost exclusively in conjunction with $b$.

The covariances of the GPs $f_s$ and $f_b$ are are of the exponential form, with distance $d(x,y)$ between locations $x$ and $y$ defined as great-circle distance. This covariance function is positive definite \citep{spherical-validity}. The amplitude parameters are denoted $\phi_s$ and $\phi_b$, respectively, and the scale parameters are denoted $\theta_s$ and $\theta_b$.

The mean of $f_s$ is modelled as zero. The mean of $f_b$ is modelled as $\beta\ 1(x\in r)$, where $\beta$ is an unknown scalar coefficient, $1(\cdot)$ is the indicator function and $r$ is the region south of the Sahara. \cite{Howes} chose this covariate because the prevalence of $b$ is known to decline sharply north of the Sahara.

The form of the prior $Q$ for the unknown scalar parameters $\phi_b$, $\theta_b$, $\phi_s$, $\theta_s$, $p_1$, $V_b$, $V_s$, $\beta$ does not affect the subsequent discussion, and is suppressed for brevity. As in all Bayesian analysis, these unknown parameters are inferred along with the actual inferential targets. The prior is, of course, written out in code in \code{model.py}. 
