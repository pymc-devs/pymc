...
Because a Gaussian process is an infinite-dimensional variable, it is not feasible to compute anything like a probability density function for it. \code{GaussianProcess} objects therefore have no \code{logp} attribute, and \code{GaussianProcess} objects cannot be handled by \pkg{PyMC}'s standard MCMC machinery. However, the evaluation $f(x_*)$ of Gaussian process $f$ on mesh $x_*$ is a simple multivariate normal random variable, which has a \code{logp} attribute and can be handled by the standard machinery. If $f(x_*)$ is incorporated in the model as a variable, a minor extension to the standard machinery (implemented by this package) makes it possible to handle $f$ itself as well.
