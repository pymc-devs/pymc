...
!$OMP& PRIVATE(j,i,BK,b,dd_here,GA,prefac,snu,fl,rem,N)
...
cf2py logical intent(in), optional:: symm=0
      ...
      INTEGER nx,ny,i,j
      DOUBLE PRECISION C(nx,ny), thisC
      LOGICAL symm
      INTEGER b(2), oppt, nt, ntm
...
!$OMP PARALLEL
!$OMP& SHARED(C,nx,ny,chunk) PRIVATE(i,j,b)
...
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(i,j,b)
...
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(i,j,t,b)
