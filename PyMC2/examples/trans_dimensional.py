...
K_true = 5
T=50
A_true = exp(arange(0,-K_true,-1,dtype=float))
...
@parameter
def A(value=A_init, mu=ones(K_max,dtype=float), tau=ones(K_max,dtype=float)):
    """A ~ normal(mu, tau)"""
...
oneatatime_scales = {X: .01, A: .01}
J = JointMetropolis([X, A])
