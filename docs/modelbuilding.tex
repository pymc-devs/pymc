...
\begin{description}
    \item \texttt{sample_likelihood(iter)}: Returns the following: 
    \begin{itemize}
        \item \texttt{iter} samples from the unconditional distribution of $p(\mathtt{data} |\mathtt{stochastics})$ times the product of the probabilities or densities associated with all factor potentials.
        \item \texttt{iter} samples from the unconditional distribution of the product of the probabilities or densities associated with all factor potentials.
    \end{itemize} The mean of the former divided by the mean of the latter converges to the model likelihood, $p(\mathtt{data})$. This mean is computed by the function \texttt{weight}, but be careful; for models with more than a handful of parameters, this estimate is likely to be poor.
