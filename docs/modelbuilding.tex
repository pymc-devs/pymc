...
\begin{description}
    ...
    \item[\code{dtype}:] A NumPy dtype object (such as \code{np.int}) that specifies the type of the variable's value to fitting methods. If this is \code{None} (default) then no type is enforced.
...
\begin{description}
   ...
   \begin{description}
      ...
      \sum_i x_i^T y_i.
      \]
      \item[\code{Index}:] Has two parents $x$ and \code{index}. $x$ must be iterable, \code{index} must be valued as an integer. Index returns 
      \[
      x[\mathtt{index}].
      \]
   ...
   \item[Elementary operations on variables] Certain elementary operations on variables create \texttt{Deterministics}. For example:   
\begin{verbatim}

>>> x = MvNormalCov('x',ones(3),eye(3))
>>> y = MvNormalCov('y',ones(3),eye(3))
>>>
>>> print x+y
