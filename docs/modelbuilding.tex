...
\begin{description}
    ...
    \item[\code{dtype}:] A NumPy dtype object (such as \code{np.int}) that specifies the type of the variable's value. The variable's value is always cast to this type. If this is \code{None} (default) then no type is enforced.
...
\begin{verbatim}
>>> x = pm.MvNormalCov('x',np.ones(3),np.eye(3))
>>> y = pm.MvNormalCov('y',np.ones(3),np.eye(3))
>>>
>>> print x+y
...
for i in range(1,N):
   ...
   x[i] = pm.Normal('x_%i' % i, mu=x[-1], tau=1)
