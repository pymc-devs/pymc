...
Although the data are modelled with statistical distributions, just as random variables are, their values are immutable (since they have been observed). Data are represented by \code{Stochastic} objects whose \code{observed} attribute is set to \code{True}. If a stochastic variable's \code{observed} flag is \code{True}, its value cannot be changed, and it won't be sampled by the fitting method..

\subsection*{Declaring stochastic variables to be data}

In the short and long interfaces, a \code{Stochastic} object's \code{observed} flag can be set to true by stacking a \code{@observed} decorator on top of the \code{@stochastic} decorator:
\begin{verbatim}
@observed
@stochastic(dtype=int)
def D(value = count_array, switchpoint = s, early_rate = e, late_rate = l):
    ...
    return logp
\end{verbatim}
In the automatic and direct interfaces, the \code{observed} argument can be simply set to \code{True}.
