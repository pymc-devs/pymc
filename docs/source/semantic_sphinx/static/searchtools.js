...
var Search = {
  ...
  performSearch: function (query) {
    ...
    this.output = $('<div class="ui container"/>').appendTo(this.out);
    ...
    function displayNextItem() {
      ...
      if (results.length) {
        ...
        if (DOCUMENTATION_OPTIONS.FILE_SUFFIX === '') {
          ...
          listItem.append($('<div class="ui medium header"></div>').html(
            $('<a/>').attr('href',
              DOCUMENTATION_OPTIONS.URL_ROOT + dirname +
              highlightstring + item[2]).html(header))
          );
        } else {
    ...
    for (var prefix in objects) {
      for (var name in objects[prefix]) {
        ...
        if (fullname.toLowerCase().indexOf(object) > -1) {
          ...
          if (fullname.toLowerCase() == object || parts[parts.length - 1].toLowerCase() == object) {
  ...
  makeSearchSummary: function (htmlText, keywords, hlwords) {
    ...
    var rv = $('<div class="ui message"></div>').text(excerpt);
    $.each(hlwords, function () {
      rv = rv.highlightText(this, 'ui teal label');
    });
    return rv;
  }
};
