...
s = 1#AddVar(model, s, Uniform(.01, 10))

g = T.constant(group)

#y ~ Normal(m[g] * p, s)
AddVar(model, T.constant(y), Normal( sum(effects[g] * predictors, 1),1))#s[g]))
...
hmc_cov = approx_cov( view, chain) #find a good orientation using the hessian at the MAP

step_method = CompoundStep([hmc.HMCStep(view, hmc_cov)])
