...
def test_simple_model():
    ...
    with pm.Model(coords=coords) as model:
        x = pmx.Normal("x", mu=1, dims=("a", "b"))
        sigma = pmx.HalfNormal("sigma", dims=("a",))
        y = pmx.Normal("y", mu=x.T * 2, sigma=sigma, dims=("b", "a"))

    with pm.Model(coords=coords) as xmodel:
        x = pmx.Normal("x", mu=1, dims=("a", "b"))
        sigma = pmx.HalfNormal("sigma", dims=("a",))
        # Imply a transposition
        y = pmx.Normal("y", mu=x * 2, sigma=sigma, dims=("b", "a"))
...
def test_complex_model():
    ...
    with pm.Model(coords=coords) as xmodel:
        x = pmx.Data("x", x_np, dims="obs")
        y = pmx.Data("y", y_np, dims="obs")
        knots = pmx.Data("knots", knots_np, dims=("knot",))
        group_idx = pmx.math.as_xtensor(group_idx_np, dims=("obs",))

        sigma = pmx.HalfCauchy("sigma", beta=1)
        sigma_beta0 = pmx.HalfNormal("sigma_beta0", sigma=10)
        beta0 = pmx.HalfNormal("beta_0", sigma=sigma_beta0, dims=("group",))
        z = pmx.Normal("z", dims=("group", "knot"))

        delta_factors = pmx.math.softmax(z, dim="knot")
        slope_factors = 1 - delta_factors.isel(knot=slice(None, -1)).cumsum("knot")
        spline_slopes = pmx.math.concat([beta0, beta0 * slope_factors], dim="knot")
        beta = pmx.math.concat([beta0, spline_slopes.diff("knot")], dim="knot")

        beta = pm.Deterministic("beta", beta)

        X = pmx.math.maximum(0, x - knots)
        mu = (X * beta.isel(group=group_idx)).sum("knot")
        y_obs = pmx.Normal("y_obs", mu=mu, sigma=sigma, observed=y)
