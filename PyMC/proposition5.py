...
DANGEROUS THINGS:
    ...
    - += is an inplace operation, so it probably doesn't call the attribute set stuff. 
"""
...
A useful addition would be to allow the MC loop to sample each model according
to its weight. So if a model is clearly not in the game, it is sampled less 
often than models that have a higher probability. Does that make sense ?
"""
